...
noto_sans_regular = os.path.join(font_dir, 'NotoSansJP-Regular.otf')
noto_sans_bold = os.path.join(font_dir, 'NotoSansJP-Bold.otf')
noto_sans_black = os.path.join(font_dir, 'NotoSansJP-Black.otf')
...
credits_json_file = os.path.join(current_dir, '..', 'assets', 'scenario', 'staff_credits_ver.3.json')
...
def play_music(music_file, loops=-1):
    ...
    music_path = os.path.abspath(os.path.join(DEFAULT_MUSIC_PATH, music_file))  # 常に絶対パスを使用
...
# JSONファイルのパスを絶対パスから相対パスに変換する関数
def convert_paths_to_relative(data):
    for page in data["story"]:
        for flag in page["flags"]:
            if "bg_change" in flag:
                flag["bg_change"] = os.path.relpath(flag["bg_change"], current_dir)
            if "music" in flag:
                flag["music"] = os.path.relpath(flag["music"], current_dir)
            if "sound_effect" in flag:
                flag["sound_effect"] = os.path.relpath(flag["sound_effect"], current_dir)
            if "voice" in flag:
                flag["voice"] = os.path.relpath(flag["voice"], current_dir)
        for segment in page["segments"]:
            if "bg_change" in segment:
                segment["bg_change"] = os.path.relpath(segment["bg_change"], current_dir)
            if "music" in segment:
                segment["music"] = os.path.relpath(segment["music"], current_dir)
            if "sound_effect" in segment:
                segment["sound_effect"] = os.path.relpath(segment["sound_effect"], current_dir)
            if "voice" in segment:
                segment["voice"] = os.path.relpath(segment["voice"], current_dir)
            if "choices" in segment:
                for choice in segment["choices"]:
                    choice["choice_file"] = os.path.relpath(choice["choice_file"], current_dir)
...
def convert_paths_to_absolute(data):
    for page in data["story"]:
        for flag in page["flags"]:
            if "bg_change" in flag:
                flag["bg_change"] = os.path.join(current_dir, flag["bg_change"])
            if "music" in flag:
                flag["music"] = os.path.join(current_dir, flag["music"])
            if "sound_effect" in flag:
                flag["sound_effect"] = os.path.join(current_dir, flag["sound_effect"])
            if "voice" in flag:
                flag["voice"] = os.path.join(current_dir, flag["voice"])
        for segment in page["segments"]:
            if "bg_change" in segment:
                segment["bg_change"] = os.path.join(current_dir, segment["bg_change"])
            if "music" in segment:
                segment["music"] = os.path.join(current_dir, segment["music"])
            if "sound_effect" in segment:
                segment["sound_effect"] = os.path.join(current_dir, segment["sound_effect"])
            if "voice" in segment:
                segment["voice"] = os.path.join(current_dir, segment["voice"])
            if "choices" in segment:
                for choice in segment["choices"]:
                    choice["choice_file"] = os.path.join(current_dir, choice["choice_file"])
...
def save_game(slot):
    ...
    # 相対パスに変換してから保存
    convert_paths_to_relative(story_data)
